...
exports.getClients = asyncHandler(async (req, res, next) => {
  const { Client, Project } = req.tenant.models;
...
exports.getClient = asyncHandler(async (req, res, next) => {
  ...
  const ownerIds = (client.projects || []).map(p => p.ownerId);
  const userMap = await req.tenant.getUsers(ownerIds);
  if (client.projects) {
    client.projects.forEach(p => p.setDataValue('owner', userMap[p.ownerId] || null));
  }
